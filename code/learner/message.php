...
$result = $link->query ("SELECT user2 FROM friendship WHERE user1='".$_SESSION["session_lusername"]."'");					//ανάκτηση username φίλων χρήστη από τον πίνακα user
?>
...
<?php
$result = $link->query ("SELECT count(*) AS inbox FROM message WHERE to_user='".$_SESSION["session_lusername"]."' GROUP BY to_user");		//ανάκτηση αριθμού εισερχόμενων μηνυμάτων χρήστη από τον πίνακα message
...
$limit = ($pagenum - 1) * 10;
$result = $link->query ("SELECT * FROM message WHERE to_user='".$_SESSION["session_lusername"]."' ORDER BY id DESC LIMIT ".$limit.",10");	//ανάκτηση 10 το πολύ εισερχόμενων μηνυμάτων χρήστη από τον πίνακα message
while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {											//για κάθε εισερχόμενο μήνυμα
	...
	if ($row["seen"] == 0) {														//αν δεν έχει διαβαστεί
		echo "<a href='show_message.php?id=".$row["id"]."' class='link_to_page'> <div class='not_seen_message_box'>".
		...
		echo "<a href='show_message.php?id=".$row["id"]."' class='link_to_page'> <div class='message_box'>".
		"<div class='message_info'>".$from_user."</div>"."<div class='message_info'>".$row["subject"]."</div>"."<div class='message_info'>".date("H:i:s | d-m-Y", strtotime($row["date"]))."</div>";
		echo "</div> </a>";
	}
}
