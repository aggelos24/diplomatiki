...
if (empty(mysqli_fetch_array($result, MYSQLI_ASSOC))) {																	//αν δεν υπάρχει
	...
	$link->query ("UPDATE section SET title='".$_POST["title"]."' WHERE number=".$_POST["number"]);						//ενήμερωση πίνακα section
	$result->free();
	$link->close();																										//κλείσιμο σύνδεσης με βάση
	echo "<script> alert('Η ενότητα μετονομάστηκε.'); location.href = 'content.php'; </script>";						//εμφάνιση κατάλληλου μηνύματος και επιστροφή στη σελίδα content.php
}
?>