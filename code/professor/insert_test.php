...
if (empty(mysqli_fetch_array($result, MYSQLI_ASSOC))) {																								//αν δεν υπάρχει
	...
	echo "<script> alert('Δεν υπάρχει μαθητής με αυτό το Όνομα Χρήστη.'); location.href = 'insert_test_form.php'; </script>";						//εμφάνιση κατάλληλου μηνύματος και επιστροφή στη σελίδα insert_test_form.php
}
if ($_POST["section_number"] > 0) {
	...
	if (empty(mysqli_fetch_array($result, MYSQLI_ASSOC))) {																							//αν δεν υπάρχει
		$result->free();
		$link->close();																																//κλείσιμο σύνδεσης με βάση
		echo "<script> alert('Δεν υπάρχει ενότητα με αυτό τον αριθμό.'); location.href = 'insert_test_form.php'; </script>";						//εμφάνιση κατάλληλου μηνύματος και επιστροφή στη σελίδα insert_test_form.php
	}
	$minimum_question_num = 4;
	$result = $link->query ("SELECT count(*) AS difficult_num FROM question_and_answer WHERE difficult=1 AND section_number=".$section_number);		//ανάκτηση αριθμού δύσκολων ερωτημάτων από τον πίνακα question_and_answer
	$row = mysqli_fetch_array($result, MYSQLI_ASSOC);
	if ($row["difficult_num"] < $minimum_question_num) {																							//αν ο αριθμός των δύσκολων ερωτημάτων είναι μικρότερος του ελάχιστου
		$result->free();
		$link->close();																																//κλείσιμο σύνδεσης με βάση
		echo "<script> alert('Δεν υπάρχουν αρκετές δύσκολες ερωτήσεις.'); location.href = 'insert_test_form.php'; </script>";						//εμφάνιση κατάλληλου μηνύματος και επιστροφή στη σελίδα insert_test_form.php
	}
	$result = $link->query ("SELECT count(*) AS easy_num FROM question_and_answer WHERE difficult=0 AND section_number=".$section_number);			//ανάκτηση αριθμού εύκολων ερωτημάτων από τον πίνακα question_and_answer
	$row = mysqli_fetch_array($result, MYSQLI_ASSOC);
	if ($row["easy_num"] < $minimum_question_num) {																									//αν ο αριθμός των δύσκολων ερωτημάτων είναι μικρότερος του ελάχιστου
		$result->free();
		$link->close();																																//κλείσιμο σύνδεσης με βάση
		echo "<script> alert('Δεν υπάρχουν αρκετές εύκολες ερωτήσεις.'); location.href = 'insert_test_form.php'; </script>";						//εμφάνιση κατάλληλου μηνύματος και επιστροφή στη σελίδα insert_test_form.php
	}
}
else if ($_POST["section_number"] == 0) {																											//αν το τεστ είναι εφ' όλης της ύλης
	$section_number = "NULL";
	$minimum_question_num = 9;
	$result = $link->query ("SELECT count(*) AS difficult_num FROM question_and_answer WHERE difficult=1");											//ανάκτηση αριθμού δύσκολων ερωτημάτων από τον πίνακα question_and_answer
	$row = mysqli_fetch_array($result, MYSQLI_ASSOC);
	if ($row["difficult_num"] < $minimum_question_num) {																							//αν ο αριθμός των δύσκολων ερωτημάτων είναι μικρότερος του ελάχιστου
		$result->free();
		$link->close();																																//κλείσιμο σύνδεσης με βάση
