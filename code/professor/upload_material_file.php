...
if (!empty(mysqli_fetch_array($result, MYSQLI_ASSOC))) {																		//αν υπάρχει
	...
	echo "<script> alert('Υπάρχει αρχείο με αυτό το όνομα.'); location.href = 'material.php'; </script>";						//εμφάνιση κατάλληλου μηνύματος και επιστροφή στη σελίδα material.php
}
if ($ok) {
	if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file)) {														//απόπειρα μετακίνησης αρχείου στην διεύθυνση προορισμού
		chmod($target_file, 0777);
		$link->query ("INSERT INTO material (path, description) VALUES ('".$target_file."', '".$_POST["description"]."')");		//αν επιτυχία, εισαγωγή στον πίνακα material
		$result->free();
		$link->close();																											//κλείσιμο σύνδεσης με βάση
		echo "<script> alert('Το αρχείο ανέβηκε.'); location.href = 'material.php'; </script>";									//εμφάνιση κατάλληλου μηνύματος και επιστροφή στη σελίδα material.php
	}
	else {																														//αν αποτυχία
		$result->free();
		$link->close();																											//κλείσιμο σύνδεσης με βάση
