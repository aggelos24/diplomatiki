...
if (!empty(mysqli_fetch_array($result, MYSQLI_ASSOC))) {																		//αν υπάρχει
	...
	echo "<script> alert('Υπάρχει αρχείο με αυτό το όνομα.'); location.href = 'project.php?id=".$id."'; </script>";				//εμφάνιση κατάλληλου μηνύματος και επιστροφή στη σελίδα project.php
}
if ($ok) {
	if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file)) {														//απόπειρα μετακίνησης αρχείου στην διεύθυνση προορισμού
		chmod($target_file, 0777);
		$link->query ("INSERT INTO source_file (id, project_id, user, path, description) VALUES (DEFAULT, ".$id.", 'aggelos24', '".$target_file."', '".$_POST["description"]."')");
																																//αν επιτυχία, εισαγωγή αρχείου στον πίνακα source_file
		$result->free();
		$link->close();																											//κλείσιμο σύνδεσης με βάση
		echo "<script> alert('Το αρχείο ανέβηκε.'); location.href = location.href = 'project.php?id=".$id."'; </script>";		//εμφάνιση κατάλληλου μηνύματος και επιστροφή στη σελίδα project.php
	}
	else {																														//αν αποτυχία
		$result->free();
		$link->close();																											//κλείσιμο σύνδεσης με βάση
