...
if (empty($users)) {																																					//αν ο πίνακας είναι άδειος
    echo "<script> alert('Δεν έβαλες κανένα μαθητή στην ομάδα.'); location.href = 'group_project.php'; </script>";														//εμφάνιση κατάλληλου μηνύματος και επιστροφή στη σελίδα group_project.php
}
else if (count($users) == 1) {
    echo "<script> alert('Μια ομάδα θα πρέπει να αποτελείται από τουλάχιστον 2 άτομα.'); location.href = 'group_project.php'; </script>";								//εμφάνιση κατάλληλου μηνύματος και επιστροφή στη σελίδα group_project.php
}
...
for ($i = 0; $i < count($users); $i++) {																																//για κάθε επιλεγμένο χρήστη
	...
	$link->query ("INSERT INTO notification (id, to_user, text, seen, display) VALUES (DEFAULT, '".$users[$i]."', '".$notification_text."', DEFAULT, DEFAULT)");		//δημιουργία ειδοποίησης για το χρήστη
