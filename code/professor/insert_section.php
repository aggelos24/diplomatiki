...
if (!empty(mysqli_fetch_array($result, MYSQLI_ASSOC))) {									//αν υπάρχει
	...
	echo "<script> alert('Υπάρχει ήδη ενότητα με αυτό τον αριθμό.'); location.href = 'content.php'; </script>";		//εμφάνιση κατάλληλου μηνύματος και επιστροφή στη σελίδα content.php
}
else {																//αν δεν υπάρχει
	$link->query ("INSERT INTO section (number, title) VALUES (".$_POST["number"].", '".$_POST["title"]."')");		//εισαγωγή ενότητας στον πίνακα section
	$result->free();
	$link->close();														//κλείσιμο σύνδεσης με βάση
