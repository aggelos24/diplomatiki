...
if (!empty(mysqli_fetch_array($result, MYSQLI_ASSOC))) {																						//αν υπάρχει
	...
	echo "<script> alert('Το Όνομα Χρήστη υπάρχει ήδη, παρακαλώ επίλεξε άλλο.'); location.href = 'login_register_form.php'; </script>";			//εμφάνιση κατάλληλου μηνύματος και επιστροφή στη σελίδα login_register_form.php
}
else {																																			//αν δεν υπάρχει
	...
	$result->free();
	$link->close();																																//κλείσιμο σύνδεσης με βάση
	echo "<script> alert('Εγγράφηκες επιτυχώς, τωρά μπορείς να συνδεθείς.'); location.href = 'login_register_form.php'; </script>";				//εμφάνιση κατάλληλου μηνύματος και επιστροφή στη σελίδα login_register_form.php
}
?>